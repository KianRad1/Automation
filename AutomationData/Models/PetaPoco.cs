...
namespace PetaPoco
{
    ...
    public class Database : IDisposable
    {
        ...
        // B2S.Core.Helper to create a transaction scope
        ...
        public object Insert(string tableName, string primaryKeyName, object poco, IEnumerable<string> columns)
        {
            return Insert(tableName, primaryKeyName, true, poco, columns);
        }
        ...
        public object Insert(string tableName, string primaryKeyName, bool autoIncrement, object poco, IEnumerable<string> columns)
        {
            try
            {
                OpenSharedConnection();
                    ...
                    using (var cmd = CreateCommand(_sharedConnection, ""))
                    {
                        ...
                        if (columns == null)
                        {
                            foreach (var i in pd.Columns)
                            {
                                ...
                                //if (i.Key.ToLower() == "createdon")
                                //    continue;
                                //if (i.Key.ToLower() == "createdby")
                                //    continue;
                        ...
                        else
                        {
                            foreach (var i1 in columns)
                            {
                                ...
                                // Don't insert the primary key (except under oracle where we need bring in the next sequence value)
                                if (autoIncrement && primaryKeyName != null && string.Compare(i1, primaryKeyName, true) == 0)
                                {
                                    // Setup auto increment expression
                                    string autoIncExpression = _dbType.GetAutoIncrementExpression(pd.TableInfo);
                                    if (autoIncExpression != null)
                                ...
                                //if (i.ColumnName.ToLower() == "createdon")
                                //    continue;
                                //if (i.ColumnName.ToLower() == "createdby")
                                //    continue;

                                names.Add(_dbType.EscapeSqlIdentifier(i1));
                                values.Add(string.Format("{0}{1}", _paramPrefix, index++));
                                AddParam(cmd, i.GetValue(poco), i.PropertyInfo);
                            }
                        }
                        //if (pd.Columns.ContainsKey("CreatedOn"))
                        //{
                        //    names.Add(_dbType.EscapeSqlIdentifier("CreatedOn"));
                        //    values.Add(string.Format("{0}{1}", _paramPrefix, index++));
                        //    AddParam(cmd, DateTime.Now, pd.Columns["CreatedOn"].PropertyInfo);
                        //}
                        //if (pd.Columns.ContainsKey("CreatedBy"))
                        //{
                        //    names.Add(_dbType.EscapeSqlIdentifier("CreatedBy"));
                        //    values.Add(string.Format("{0}{1}", _paramPrefix, index++));
                        //    AddParam(cmd, "BPoco", pd.Columns["CreatedBy"].PropertyInfo);
                        //}
        ...
        public object Insert(object poco, IEnumerable<string> columns)
        {
            var pd = PocoData.ForType(poco.GetType());
            return Insert(pd.TableInfo.TableName, pd.TableInfo.PrimaryKey, pd.TableInfo.AutoIncrement, poco, columns);
        }
        public object Insert(object poco)
        {
            var pd = PocoData.ForType(poco.GetType());
            return Insert(pd.TableInfo.TableName, pd.TableInfo.PrimaryKey, pd.TableInfo.AutoIncrement, poco, null);
        }
        ...
        #region operation: join
        public AutomationData.Models.Base.Query JOIN<rootTable>(string rootColumn, string joinedColumn, object[] joinTables)
        {
            var query = new AutomationData.Models.Base.Query();
            query.q.Append(BuildSql(typeof(rootTable), joinTables, rootColumn, joinedColumn));
            return query;
        }

        public string BuildSql(Type rootType, object[] joinTypes, string rootColumn, string joinedColumn)
        {
            PocoData rootData = PocoData.ForType(rootType);
            string rootTable = rootData.TableInfo.TableName;
            ...
            {
                PocoData joinData = PocoData.ForType(joinType as System.Type);
                columns = columns.Union(GetColumns(joinData));
                join += BuildJoin(rootTable, joinData.TableInfo.TableName, rootColumn, joinedColumn);
            }

            string columnList = string.Join(", ", columns);

            return string.Format("SELECT {0} FROM {1} WITH(NOLOCK) {2}",
                                 columnList,
                                 rootTable,
                                 join);
        }

        private string BuildJoin(string rootTable, string joinedTable, string rootColumn, string joinedColumn)
        {
            return string.Format(" INNER JOIN {0} WITH(NOLOCK) ON {1}.{2} = {3}.{4}",
                                 joinedTable,
                                 rootTable,
                                 rootColumn,
                                 joinedTable,
        ...
        {
            var tableName = rootData.TableInfo.TableName;

            var cols = from c in rootData.QueryColumns
                       select tableName + "." + _dbType.EscapeSqlIdentifier(c);
            return cols;
        }
        ...
        public void BulkUpdate(IEnumerable<object> pocos)
        {
            try
            {
                object template;
                ...
                try
                {
                    using (var cmd = CreateCommand(_sharedConnection, ""))
                    {
                        pd = PocoData.ForObject(pocos.First(), primaryKeyName);
        ...
        public void Save(string tableName, string primaryKeyName, object poco)
        {
            ...
            {
                Insert(tableName, primaryKeyName, true, poco, null);
            }
            else
            {
    ...
    */
    ...
    public class ColumnInfo
    {
        ...
        public static ColumnInfo FromProperty(PropertyInfo pi)
        {
            ...
            if (ColAttrs.Length > 0)
            {
                ...
                foreach (var item in ColAttrs)
                {
                    if ((item as ResultColumnAttribute) != null)
    ...
    public class Sql
    {
        ...
        {
            return Append(new Sql("FROM " + String.Join(", ", (from x in tables select x.ToString() + " WITH(NOLOCK) ").ToArray())));
        }
    ...
    namespace Internal
    {
        ...
        {
            // B2S.Core.Helper to handle named parameters from object properties
